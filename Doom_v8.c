...
static char current_target_type = 'E';
...
static bool i2c_write_safe(i2c_inst_t *bus, uint8_t addr, const uint8_t *b, size_t n) {
...
static inline void oled_cmd(uint8_t c) {
    uint8_t p[2] = {0x80, c}; 
    i2c_write_blocking(i2c_default, OLED_ADDR, p, 2, false);
}
...
static void spawn(void){
    if(Ec < MAX_E){
        ...
        shape_t target_shape = (current_target_type == 'E') ? SQUARE : CIRCLE;
        E[Ec++] = (enemy){ 
            .k = target_shape,
            .x = xpos,
            .s = START_SZ,
...
static int update(void){
    int col = 0;
    for(int i=0; i<Ec; i++){
        if(E[i].live && (E[i].s += GROWTH) >= COLL_SZ) 
            col = 1;
    }
    return col;
}
...
static void update_crosshair(void){
    ...
    jx=fmaxf(-1,fminf(1,jx)); 
    jy=fmaxf(-1,fminf(1,jy));
...
static void render_world(void){
    ...
    update_crosshair();
    for(int i=-2;i<=2;i++){ 
        px(cross_x+i, cross_y,1); 
        px(cross_x, cross_y+i,1); 
    }
    // draw enemies    
...
int Doom_v8_game(char target_type) {
    ...
    sleep_ms(400);
    adc_select_input(0); 
    center_x_raw=adc_read();
    adc_select_input(1); 
    center_y_raw=adc_read();
    cross_x = W/2; 
    cross_y = H/2;
    Ec=0; 
    memset(E,0,sizeof E); 
